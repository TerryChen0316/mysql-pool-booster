...
PoolConnectionManager.prototype.executeCallback = function (callback, connection) {
    ...
    if (changedUser) {
        ...
        connection.getRawConnection().config = this._config.newConnectionConfig();
...
PoolConnectionManager.prototype.createNewConnection = function (callback) {
    ...
    var connection = new PoolConnection(pool, {
        config: this._config.newConnectionConfig()
    });
...
PoolConnectionManager.prototype.purgeConnection = function (connection, callback) {
    ...
    var isDisconnected = connection.getRawConnection().state === 'disconnected';
    if (isDisconnected) {
        connection.destroy();
    }
    ...
    if (!isDisconnected && !connection.getRawConnection()._protocol._quitSequence) {
